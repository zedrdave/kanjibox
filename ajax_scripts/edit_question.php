...
if(isset($params['question_id'])) {
	...
	if(isset($_REQUEST['add_wrong_answer'])) {
		...
		if($jmdict_id == $question->jmdict_id)
			...
			$res = mysql_query("SELECT COUNT(*) AS count FROM grammar_answers WHERE jmdict_id = $jmdict_id AND question_id = $question->question_id") or die(mysql_error());
	...
	<p><a class="delete-button" href="#" onclick="delete_question(<?php echo $question->question_id ?>); return false;">×</a> ID: <span id="question-id"><?php echo $question->question_id ?></span> 
	...
	<p id="question-str"><?php echo $sent->example_str ?></p>
		...
		<p>Answer position: <a href="#" onclick="move_selection(-1); highlight_text(); return false;">←</a> <a href="#" onclick="extend_selection(-1); highlight_text(); return false;">-</a> <input type="hidden" name="pos_start" id="pos_start" value="<?php echo $question->pos_start ?>" />[<span id="pos_start_txt" /> ~ <span id="pos_end_txt" />]<input type="hidden" name="pos_end" id="pos_end" value="<?php echo $question->pos_end ?>" onchange="show_question_save_button();" /> <a href="#" onclick="extend_selection(1); highlight_text(); return false;">+</a> <a href="#" onclick="move_selection(1); highlight_text(); return false;">→</a><?php
	if($question->pos_start == 0 && $question->pos_end == 0)
		echo ' <span class="notice">(set answer position)</span>';
	else {
		$substr = mb_substr($sent->example_str, $question->pos_start, $question->pos_end - $question->pos_start, 'utf-8');
...
<script type="text/javascript">
	...
	$(document).ready(function()  {
		...
		$('#wrong-answer-form').ajaxForm({
			...
			beforeSubmit: function() { $('#wrong-answer-results').html('reloading...') },
		});	
		
		$('#save-question-data-form').ajaxForm({
			target: '#question > .ajax-result',
			beforeSubmit: function() { $('#question > .ajax-result').html('reloading...') },
		});	
