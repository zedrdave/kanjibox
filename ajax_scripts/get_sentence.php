...
if(isset($_REQUEST['id'])) {
	...
	if(@$_REQUEST['word'])
		$query_where .= ' AND e.example_str LIKE \'%' . mysql_real_escape_string($_REQUEST['word']) . '%\'';
	if(@$_REQUEST['not_word'])
		$query_where .= ' AND e.example_str NOT LIKE \'%' . mysql_real_escape_string($_REQUEST['not_word']) . '%\'';
	
	$res = mysql_query('SELECT COUNT(*) AS count '. $query_body . $query_where) or die(mysql_error());
...
if($count == 1) {
	...
	if(@$params['edit']) {
		...
		while($part = mysql_fetch_object($res_parts)) {
			...
			if(mb_strlen($sent_part) > 1) {
				...
				for($i = 1; $i < mb_strlen($sent_part); $i++)
					$parts_html .= "<li style=\"display:none;\"><a href=\"#\" onclick=\"split_sentence_part($sentence->example_id, $part->pos_start, $i, '#part_" . $sentence->example_id . "_" . $part->pos_start . "'); return false;\">Split as: " . mb_substr($sent_part, 0, $i) . " + " . mb_substr($sent_part, $i) . "</a></li>";
		...
		echo "<span onclick=\"\$(this).hide(); \$('#jp_edit_$sentence->example_id').show()\" id=\"sentence_jp_$sentence->example_id\" class=\"sentence_jp\">$parsed_sent_jp</span><input type=\"edit\" id=\"jp_edit_$sentence->example_id\" name=\"example_str\" size=\"60\" style=\"display:none;\" value=\"" . str_replace('"', '\"', $sentence->example_str) . "\" onchange=\"update_sentence_str($sentence->example_id, this.value);\" onblur=\"\$(this).hide(); \$('#sentence_jp_$sentence->example_id').show();\"></input>";
		...
		function confirm_delete_answer(id, jmdict_id)  
		{
			if (confirm('Are you SURE you want to delete this question?')) {
				$.get('<?php echo SERVER_URL ?>ajax/edit_sentence/?delete_id=' + id + '&delete_jmdict_id=' + jmdict_id + '&short_reply=1', function (data) {
					...
					$('#delete_sent_' + id).css('text-decoration', 'line-through');
					setTimeout(function() { $('#ajax-result').html('') }, 5000);
				});
			}
		}
...
<?php
		...
		while($question = mysql_fetch_object($questions_res)) {
			echo "<li><i>" . mb_substr($sentence->example_str, 0, $question->pos_start, "utf-8") . '<span class="small-highlighted">'. mb_substr($sentence->example_str, $question->pos_start, $question->pos_end - $question->pos_start, "utf-8") . '</span>' . mb_substr($sentence->example_str, $question->pos_end, 1000, "utf-8") . "</i> (N" . $question->njlpt . ") [<a href=\"#\" onclick=\"load_question(" . $question->question_id . "); return false;\">edit</a>]</li>";
		}
