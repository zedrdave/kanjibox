...
if ($count == 1) {
    ...
    if (@$params['edit']) {
        ...
        while ($part = mysql_fetch_object($res_parts)) {
            ...
            $res_alt = mysql_query("SELECT j.*, jx.gloss_english AS gloss FROM jmdict j LEFT JOIN jmdict_ext jx ON jx.jmdict_id = j.id WHERE j.id != " . $part->jmdict_id . " AND (j.word = '" . DB::getConnection()->quote($part->word) . "' OR j.reading = '" . DB::getConnection()->quote($part->reading) . "'" . ($sent_part != $part->word ? " OR j.word = '" . DB::getConnection()->quote($sent_part) . "'" : '') . ($sent_part != $part->reading ? " OR j.reading = '" . DB::getConnection()->quote($sent_part) . "'" : '') . ")") or die(mysql_error());
