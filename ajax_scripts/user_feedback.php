...
if(@$params['type'] != 'general' && !@$_SESSION['cur_session'])
	...
	foreach($form_options as $id => $options)
	{
        ...
        // $submit_cond = '';
        // for($i = 1; $i <= 3; $i++)
        //     if(@$options['param_' . $i . '_required'])
        //         $submit_cond .= '($(\'#form_' . $id . '_param_'. $i . '\').val() == \'\') || ';
        // if($submit_cond)
        //     $submit_conds .= ('$("#feedback_options_' . $id . '").submit(function(){ if( ' . $submit_cond . ' false) { alert(\'Please fill up the necessary fields completely before submitting.\'); return false; } else alert($(\'#form_' . $id . '_param_1\').val()); return true; })' . ";\n");

        // $submit_conds .= ('$("#feedback_options_' . $id . '").submit(function(){ alert(\'DEBUG: \' + $(\'#form_' . $id . '_param_1\').val()); return true; })' . ";\n");
		...
		if(is_array($options['param_1']))
			$forms .=  $options['param_1_title'] . ' ' . get_select_menu($options['param_1'], 'form_' . $id . '_param_1', '', '', '...', 'param_1', (@$options['param_1_required'] ? 'form_required' : ''));
		else
		...
		if(@$options['param_2'])
		{
			...
			$forms .= ' ' . get_select_menu($options['param_2'], 'form_' . $id . '_param_2', '', '', '...', 'param_2', (@$options['param_2_required'] ? 'form_required' : ''));
		}

		if(@$options['param_3'])
		{
			if(is_array($options['param_3']))
				$forms .=  $options['param_3_title'] . ' ' . get_select_menu($options['param_3'], 'param_3', '', '', '...', (@$options['param_3_required'] ? 'form_required' : ''));
			else
