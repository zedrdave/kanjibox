...
if(! $row)
	...
	if(@$_REQUEST['learn_vocab']) {
		mysql_query("INSERT IGNORE INTO jmdict_learning (user_id, jmdict_id, date_first) VALUES ($user_id, $jmdict_id, NOW())") or die(mysql_error());
		mysql_query("UPDATE jmdict_learning SET total = total+1, curve = LEAST(2000, tan(atan(curve/1000-1)+0.15)*1000+1000) where `user_id` = $user_id AND jmdict_id = $jmdict_id") or die(mysql_error());
	}
