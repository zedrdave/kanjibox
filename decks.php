...
if (!init_app()) {
    ...
    display_login_page(SERVER_URL . 'decks.php?' . $_SERVER['QUERY_STRING'],
        'To use <a href="https://www.facebook.com/kanjibox/">KanjiBox Decks</a>, please: ');
    exit();
}
?><!DOCTYPE html>
<html>
    ...
    <body>
        ...
        <?php
        $type = (!empty($_REQUEST['type']) && $_REQUEST['type'] == 'vocab' ? 'vocab' : 'kanji');
        if (!empty($_REQUEST['njlpt'])) {
            $njlpt = min(5, max(1, $_REQUEST['njlpt']));
        } else {
            $njlpt = max(1, $_SESSION['user']->getLevel());
        }
        $include_below = (!empty($_REQUEST['include_below']) && $_REQUEST['include_below'] == 1);
        $curve_values = [1500 => 'Very Bad', 1050 => 'Bad', 950 => 'OK', 500 => 'Good', 0 => 'Very Good'];

        if (!empty($_REQUEST['curve'])) {
        ...
        if (!empty($_REQUEST['num_examples'])) {
