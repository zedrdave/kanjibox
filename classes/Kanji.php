...
class Kanji extends Question
{
	...
	function get_db_data($how_many, $grade, $user_id = -1, $cumulative = false, $options = 0) {
		...
		for($i = 0; $i < count($picks) - 1; $i+=2)
		{
			...
			$choice[0] = $picks[$i];
			$choice[2] = $picks[$i+1];
			$choice[1] = $this->get_other_kanji($choice[0]->id, $grade, array($choice[2]->id, $choice[0]->id), 1, $options);
			$choice[3] = $this->get_other_kanji($choice[2]->id, $grade, array($choice[0]->id, $choice[1]->id, $choice[2]->id), 1, $options);
	...
	static function get_random_kanjis ($grade1 = -1, $grade2 = -1, $how_many = 1, $exclude = NULL, $options = 0) 
	{		
		if ($grade1[0] == 'N')
		{
			$grade1 = $grade1[1];
...
//STATIC needed for use by multiplayer
	static function get_random_weighted_kanjis($user_id, $grade1 = -1, $grade2 = -1, $how_many = 1, $query_where_extra = '')
	{
		if ($grade1[0] == 'N')
		{
			$grade1 = $grade1[1];
		...
		$query = "SELECT  k.`id`, k.`kanji`, k.`traditional`, `prons`, " . Kanji::get_query_meaning() . ", IF(l.curve IS NULL, 1000, curve)+1000*rand() as xcurve from kanjis k LEFT JOIN kanjis_ext kx ON kx.kanji_id = k.id left join learning l on l.user_id = '" . (int) $user_id . "' AND k.id = l.kanji_id WHERE 1 ";
	  ...
	  if ($grade1 > 0)
	  {
		...
		if($translator_mode) {
			if($_SESSION['user']->is_on_translator_probation())
				...
				$query .= ' AND (kx.meaning_' . Kanji::$lang_strings[$_SESSION['user']->get_pref('lang', 'kanji_lang')] . ' IS NULL OR kx.meaning_' . Kanji::$lang_strings[$_SESSION['user']->get_pref('lang', 'kanji_lang')] . " = '' OR kx.meaning_" . Kanji::$lang_strings[$_SESSION['user']->get_pref('lang', 'kanji_lang')] . " LIKE '(~)%')";
		}
	...
	function get_set_weighted_kanjis($user_id, $how_many = 1)
	{
		...
		if($translator_mode)
			$query .= ' AND (kx.meaning_' . Kanji::$lang_strings[$_SESSION['user']->get_pref('lang', 'kanji_lang')] . ' IS NULL OR kx.meaning_' . Kanji::$lang_strings[$_SESSION['user']->get_pref('lang', 'kanji_lang')] . " = '')";
	...
	static function get_similar_kanjis ($kanji_id, $howmany = 1, $grade1 = -1, $grade2 = -1, $exclude = 0, $query_where_extra = '', $options = 0) 
	{
	//	$main_rad_coef = 10;
	//	$classic_rad_coef = 30;
	//	$multi_rad_coef = 1;
...
k.`njlpt` 
	...
	static function get_meaning_str($id, $trad = false)
	{
	...
	static function get_pronunciations($db_rec)
	{
	...
	function get_kanji_examples_str($id)
	{
	...
	static function get_query_meaning()
	{
		global $_SESSION;
