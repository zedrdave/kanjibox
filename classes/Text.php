...
class Text extends Vocab
{
        ...
        $solution_id = -1)
    {
        ...
        if ($hide_pos_start >= 0 && $hide_pos_end >= 0) {
            $hint_str = mb_substr($sentence->example_str, 0, $hide_pos_start) . str_repeat('^',
                    $hide_pos_end - $hide_pos_start) . mb_substr($sentence->example_str, $hide_pos_end);
            $answer_str = $hint_str;
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            if (($row['pos_start'] >= $hide_pos_start && $row['pos_start'] < $hide_pos_end)
                ...
                if ($row['proper_noun'] || ($row['need_furi'] && !$row['jmdict_id'])) {
                    $word_brokendown = preg_replace('/([^\\x{3040}-\\x{30FF}])/u',
        ...
        if ($hide_pos_start >= 0 && $hide_pos_end >= 0) {
            ...
            $answer_str = preg_replace('/(\^)+/',
                '<span class="hilite">' . mb_substr($sentence->example_str, $hide_pos_start,
                    $hide_pos_end - $hide_pos_start) . '</span>', $answer_str);
        }
    ...
    function displayCorrection($answer_id)
    {
        ...
        $display_word_solution = preg_replace('/([^\\x{3040}-\\x{30FF}])/ue',
            "'<span class=\"kanji_detail\" href=\"#\" onclick=\"show_kanji_details(\'\\1\', \''. SERVER_URL . 'ajax/kanji_details/kanji/'.urlencode('\\1').'\');  return false;\">\\1</span>'",
            $solution->word);
