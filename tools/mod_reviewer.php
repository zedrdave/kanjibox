...
if (!empty($_REQUEST['user_id'])) {
    ...
    $query .= implode(',', explode(',', $_REQUEST['user_id']));
    $query .= ') ';
}
...
if (!empty($_REQUEST['need_work']) && $_REQUEST['need_work'] === '1') {
...
<form action="<?php echo SERVER_URL;?>tools/mod_reviewer/" class="search_form">
    ...
    User Id(s): <input name="user_id" size="30" value="<?php echo (!empty($_REQUEST['user_id']) ? $_REQUEST['user_id'] : '');?>" /> (your ID: <?php echo $_SESSION['user']->getID();?>)<br/>
    Table: <?php
    echo get_select_menu(['' => 'all', 'examples' => 'examples', 'examples_str' => 'examples_str', 'example_parts' => 'example_parts', 'jmdict' => 'jmdict', 'jmdict_ext' => 'jmdict_ext', 'kanjis_ext' => 'Kanji Translations'],
        'table_name', (!empty($_REQUEST['table_name']) ? $_REQUEST['table_name'] : ''));

    echo '<br/>';
    $select_id = 'reviewed';
    echo '<select id="' . $select_id . '" name="' . $select_id . '">';
    foreach (['' => 'all', '1' => 'Reviewed', '0' => 'Not reviewed'] as $val => $name) {
...
if ($query) {
    ...
    try {
        ...
        $stmt->execute();
        $update = $stmt->fetchObject();

        $i = 0;
        echo '<p>';

        while ($update) {
            $nextUpdate = $stmt->fetchObject();
            if ($nextUpdate) {
                $next_col = "$nextUpdate->table_name $nextUpdate->id_name $nextUpdate->id_value";
            } else {
                $next_col = '';
            }
            ...
            $thisCol = "$update->table_name $update->id_name $update->id_value";
            if ($last_col != $thisCol) {
                echo "</p><p class=\"update_chain\"><strong>$update->id_value: ";
                if (!empty($_REQUEST['details'])) {
                    switch ($update->table_name) {
                        case 'examples':
                            $query = 'SELECT * FROM examples WHERE example_id = :example_id';
                            try {
                                $stmt = DB::getConnection()->prepare($query);
                                $stmt->bindValue(':example_id', $update->id_value, PDO::PARAM_INT);
                                $stmt->execute();
                                $row_d = $stmt->fetchObject();
            ...
            echo "<a id=\"update_to_$update->update_id\" class=\"update update_replac" . ($nextUpdate && $nextUpdate->applied && $thisCol == $next_col ? 'ed' : 'ing') . ($update->need_work ? '_need_work' : '') . ($update->applied ? ($update->reviewed ? '' : '_not_reviewed') : '_not_applied') . "\" href=\"#\" onclick=\"" . ($update->applied && $update->reviewed ? 'invalidate' : 'validate') . "($update->update_id, 0); return false;\">" . (empty($update->new_value) && $update->new_value !== '0' ? '∅' : $update->new_value ) . "</a>";
            ...
            if ($_REQUEST['userlinks']) {
                if ($update->first_name[0] || $update->last_name[0]) {
                    echo ' ' . mb_substr($update->first_name, 0, 1, $encoding = "UTF-8") . '.' . mb_substr($update->last_name,
                        0, 1, $encoding = "UTF-8") . '.';
                } else {
    ...
    function validate(revid, need_work)
    {
        ...
        $.get('<?php echo SERVER_URL?>ajax/data_updates/?validate=1&need_work=' + need_work + '&update_id=' + revid, function(data) {
            ...
            if ($('#data_update_success', data).length > 0) {
                $('#update_to_' + revid).removeAttr('onclick').click(function() {
                    invalidate(revid);
                    return false;
                });
    ...
    function revert(revid)
    {
        ...
        $.get('<?php echo SERVER_URL?>ajax/data_updates/?revert=1&update_id=' + revid, function(data) {
            ...
            if ($('#data_update_success', data).length > 0) {
                ...
                $('#rev_' + revid).removeAttr('onclick').click(function() {
                    discard(revid);
                    return false;
                });
