...
// if(! $_SESSION['user']->is_admin())
	...
	echo 'Set: ';
	display_select_menu($grammar_sets, 'set_id', $set_id, "window.location.href = 'https://kanjibox.net/kb/tools/grammar_review/set_id/' + this.value + '/';", '-');
	echo '<br/>';
...
$res_count = mysql_query("SELECT COUNT(*) AS c FROM grammar_questions sq LEFT JOIN grammar_answers ga ON ga.question_id = sq.question_id LEFT JOIN grammar_answer_reviews r ON r.question_id = sq.question_id AND r.jmdict_id = ga.jmdict_id WHERE r.question_id IS NULL " . (@$user_id ? "AND sq.user_id = $user_id" : "") . ($set_id > 0 ? " AND sq.set_id = $set_id" : "") . ($set_id == -1 ? " AND (sq.set_id <= 0 OR sq.set_id IS NULL)" : "")) or die(mysql_error());
...
while($question = mysql_fetch_object($res)) {
	...
	if($question->answer_jmdict_id > 0)
        $str = mb_substr($str, 0, $question->pos_start) . '<span class="highlighted-wrong">' . $question->decoy . '</span>' . mb_substr($str, $question->pos_end);
    else
