...
<?php
	...
	// $mail = $_SESSION['user']->get_email();
	// if(empty($mail)) {
	// 	echo '<strong>no email</strong>';
	// }
?>
	<p>News, updates, community and other cool stuff: head out to <a href="//www.facebook.com/kanjibox">KanjiBox's application page on Facebook</a> ('Become a fan' if you want automatic notifications in your feed).
	</p>
	<p>
	Got questions? <a href="<?php echo get_page_url('faq') ?>">We got answers</a>....
	</p>
	<p>For more questions (and random banter), head out to <a href="//kanjibox.net/forum/">KanjiBox's Discussion Board</a>. Use the <a href="//kanjibox.net/forum/categories/kanjibox-online">KB Online section</a> for questions/bug-reports specifically about this part of the site, but feel free to use other sections as well.
	</p>
	<p>In addition to the in-game custom Study Set sections for <a href="//kanjibox.net/kb/page/play/type/vocab/mode/sets/">Kanji</a> or <a href="//kanjibox.net/kb/page/play/type/kanji/mode/sets/">Vocab</a>, there is now a publicly accessible <a href="//kanjibox.net/kb/sets/">page for browsing and exploring the Study Sets</a> that other users have shared. If you find a set you like, one click is all it takes to bookmark it and start drilling it in KanjiBox.</p>
	...
	// if($lang_vocab == 'en' || $lang_kanji == 'en') {
	 // 
	 // 		<div style="padding: 5px; border: 1px solid #00A; background-color: #DDF;"><h3 style="margin:0 0 5px 0"><i>New:</i> KanjiBox in your language!</h3>
	 // 			KanjiBox speaks your language (or can learn it with your help)<br/><a href="http://kanjibox.net/kb/page/international/">Click here</a> for more info on using and contributing to KanjiBox in your language...</div>
	 // 		
	 // }
	...
	if($lang_vocab != 'en') {
		...
		$res = mysql_query("SELECT COUNT(*) AS c FROM jmdict j WHERE j.njlpt = $level") or die(mysql_error());
		$row = mysql_fetch_object($res);
		$tot = $row->c;
	
		$res = mysql_query("SELECT COUNT(*) AS c FROM jmdict j LEFT JOIN jmdict_ext jx ON jx.jmdict_id = j.id WHERE j.njlpt = $level AND jx.gloss_" . Vocab::$lang_strings[$lang_vocab] . ' IS NOT NULL AND jx.gloss_' . Vocab::$lang_strings[$lang_vocab] . " != ''") or die(mysql_error());
		$row = mysql_fetch_object($res);
		$translated = $row->c;

		$res = mysql_query("SELECT COUNT(*) AS c FROM jmdict j LEFT JOIN jmdict_ext jx ON jx.jmdict_id = j.id WHERE j.njlpt = $level AND jx.gloss_" . Vocab::$lang_strings[$lang_vocab] . " LIKE '(~)%'") or die(mysql_error());
		$row = mysql_fetch_object($res);
		$need_work = $row->c;
		...
		// "<div style=\"width:" . ($ratio * 5) . "px;height:16px;background-color:#" . ($ratio == 100 ? '1C6' : 'ABF') . ";border:1px solid black;float:left;color:#FFF;font-size:15px;font-weight:bold;text-align:center;margin-top:3px;overflow:visible;\" alt=\"$translated/$tot\" title=\"$translated/$tot\">"  . ($ratio > 3 ? "$ratio%" : '') .  "</div>" . ($ratio < 100 ? "<div style=\"width:" . (5 * (100 - $ratio)) . "px;height:16px;background-color:#DDD;border-top:1px solid black;border-right:1px solid black;border-bottom:1px solid black;float:left;margin-top:3px;\" alt=\"$translated/$tot\" title=\"$translated/$tot\">"  . ($ratio > 3 ? '' : "$ratio%") .  "</div>" : ''). "<div style=\"clear:both;\"></div>";
	}
	if($lang_kanji != 'en') {
		$level = $_SESSION['user']->get_level();
		$res = mysql_query("SELECT COUNT(*) AS c FROM kanjis k WHERE k.njlpt = $level") or die(mysql_error());
		$row = mysql_fetch_object($res);
		$tot = $row->c;
	
		$res = mysql_query("SELECT COUNT(*) AS c FROM kanjis k LEFT JOIN kanjis_ext kx ON kx.kanji_id = k.id WHERE k.njlpt = $level AND kx.meaning_" . Kanji::$lang_strings[$lang_kanji] . ' IS NOT NULL AND kx.meaning_' . Kanji::$lang_strings[$lang_kanji] . " != ''") or die(mysql_error());
		$row = mysql_fetch_object($res);
		$translated = $row->c;
		
		
		$res = mysql_query("SELECT COUNT(*) AS c FROM kanjis k LEFT JOIN kanjis_ext kx ON kx.kanji_id = k.id WHERE k.njlpt = $level AND kx.meaning_" . Kanji::$lang_strings[$lang_kanji] . " LIKE '(~)%'") or die(mysql_error());
		$row = mysql_fetch_object($res);
		$need_work = $row->c;
	...
	<h2>Settings</h2>
			<fieldset><legend>Login Info</legend>
				...
				<div id="login-mail">Email:  <span class="login"><?php echo $_SESSION['user']->get_email(); ?></span> <a href="#" style="font-size:90%" onclick="$('#login-mail').hide(); $('#set-login-mail').show(); return false;">[change]</a></div>
				<form id="set-login-mail" class="update-login" action="<?php echo SERVER_URL ?>ajax/update_login_info/" method="post">
					Login (your email): <input type="text" size="40" name="set_login" id="set_login" value="<?php echo $_SESSION['user']->get_email(); ?>"></input> <input type="submit" name="submit-new-login" id="submit-new-login" value="Set Login"></input>
				</form>
				<p id="set-password-link"><a href="#" onclick="$('#set-password').show();$('#set-password-link').hide();return false;">[change password]</a></p>
				<form id="set-password" class="update-login" action="<?php echo SERVER_URL ?>ajax/update_login_info/" method="post">
					Password: <input type="password" name="set_password" id="set_password" size="10"></input> |	Password again: <input type="password" name="set_password_repeat" id="set_password_repeat" size="10"></input> <input type="submit" name="submit-new-password" value="Set Password" ></input>
				</form>
				<?php
				if($_SESSION['user']->get_fb_id() == 0 || $_SESSION['user']->get_first_name() == '') {
				?>
				<p id="set-name-link">Name (<em>optional</em>): <span class="name_info" id="first_name"><?php echo $_SESSION['user']->get_first_name() ?></span> <span class="name_info" id="last_name"><?php echo $_SESSION['user']->get_last_name() ?></span> <a href="#" onclick="$('#set-name').show();$('#set-name-link').hide();return false;">[change name]</a></p>
				<form id="set-name" class="update-login" action="<?php echo SERVER_URL ?>ajax/update_login_info/" method="post">
					<input type="text" name="set_first_name" id="set_first_name" size="16" placeholder="First Name"></input> <input type="text" name="set_last_name" id="set_last_name" size="16" placeholder="Last Name"></input> <input type="submit" name="change-name" value="Save" />
				</form>
				<?php
			}		
				?>
			<script type="text/javascript">
				$(document).ready(function() {
					...
					// $(document).snowfall({flakeCount : 60, minSize : 3, maxSize : 5, shadow : false, round : true,  minSpeed : 1, maxSpeed : 2, collection : '#kb' });
			...
			<form  class="settings" method="post" action="<?php echo APP_URL; ?>">
				...
				You are training for level: <select name="level" id="level" onchange="show_and_blink('save_prefs'); return true;" clickthrough="true" ><?php 
				$levels = Session::$level_names;
				foreach($levels as $level => $label)
					echo "<option value=\"$level\"" . ($_SESSION['user']->get_level() == $level ? ' selected' : ' ') . ">$label</option>"
				?></select>
				</fieldset>
				<p style="text-align:center;"><a href="#" id="more-options-link" onclick="$('#more-options').show('slow');$(this).hide();return false;">Show all settings...</a></p>
		...
		<hr/>
		<h2><a href="#" onclick="$('#exportprintouts').slideToggle();return false;">Export printouts</a></h2>
		...
		if($_SESSION['user'])
		{
			$res = mysql_query('SELECT COUNT(*) AS c FROM messages WHERE msg_read = 1 AND user_id_to = ' . $_SESSION['user']->get_id()) or die(mysql_error());
			$row = mysql_fetch_assoc($res);
			if($row['c'] > 0)
			{
				?>
				<h2><a href="#" onclick="do_load('<?php echo SERVER_URL ?>ajax/get_messages/show/read/', 'old-kb-msgs');return false;">Past Messages</a></h2>
