...
if(isset($_SESSION['cur_session']) && 
	($_SESSION['cur_session']->get_mode() != $mode 
	|| $_SESSION['cur_session']->get_type() != $type 
	|| $_SESSION['cur_session']->get_level() != $level 
	|| ($_SESSION['cur_session']->is_drill() && isset($grade) && $_SESSION['cur_session']->get_cur_grade() != $grade)
	|| $_SESSION['cur_session']->game_over
	|| @$params['mode'] != $_SESSION['cur_session']->get_mode()
	|| ($_SESSION['cur_session']->get_mode() == SETS_MODE && @$params['set_id'] && $_SESSION['cur_session']->get_set_id() != $params['set_id'])
	|| ($_SESSION['cur_session']->get_mode() == GRAMMAR_SETS_MODE && @$params['set_id'] && $_SESSION['cur_session']->get_set_id() != $params['set_id'])
	))
{
...
elseif($mode == SETS_MODE) {		
	...
	<div id="sets_settings">
		...
		$query = 'SELECT ls.*, subs.set_id AS sub, (SELECT COUNT(*) FROM learning_set_' . ($set_type == TYPE_KANJI ? TYPE_KANJI : TYPE_VOCAB) . ' WHERE set_id = ls.set_id) AS size FROM learning_sets ls LEFT JOIN learning_set_subs subs ON subs.set_id = ls.set_id AND subs.user_id = ' . $_SESSION['user']->get_id() . ' WHERE ls.deleted = 0 AND (ls.user_id = ' . $_SESSION['user']->get_id() . " OR subs.set_id IS NOT NULL) AND set_type = '" . mysql_real_escape_string($set_type) . "' ORDER BY date_modified";
		...
		<div id="sets_editor" style="<?php if(@$params['editor'] != 'open' && !@$params['view_set_id']) echo 'display:none;' ?>">
			...
			<fieldset class="closed"><legend onclick="$(this).parent().toggleClass('closed',200);">Search public sets</legend>
				<form action="" method="post" onsubmit="$('#subscribe_results').html('Searching...'); $.post('<?php echo SERVER_URL ?>ajax/search_learning_sets/', $(this).serialize(), function (data, textStatus, jqXHR) {
...
<div id="session_frame">
	<?php
		$_SESSION['cur_session']->display_wave();
	?>
</div>
...
<?php
		if($_SESSION['cur_session']->is_drill()) {
			...
			if($options)
			{
				...
				Level: 
					<select id="options" name="options" onchange="if(this.value != '') window.location = this.value;">
				<?php
