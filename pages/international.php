...
<div id="tabs">
	...
	<div id="translators" class="scoreboard">
		...
		<h2>Vocab</h2>
		<?php
		$res = mysql_query("SELECT du.user_id, fb_id, first_name, last_name, prefs, u.privileges, COUNT(*) as c FROM `data_updates` du LEFT JOIN users u ON du.user_id = u.id LEFT JOIN users_ext ux ON ux.user_id = u.id WHERE table_name = 'jmdict_ext' AND applied = 1 AND reviewed = 1 AND need_work = 0 GROUP BY user_id ORDER BY COUNT(*) DESC LIMIT 10");
		...
		<td><h2>Kanji</h2>
		<?php
		$res = mysql_query("SELECT du.user_id, fb_id, first_name, last_name, prefs, u.privileges, COUNT(*) as c FROM `data_updates` du LEFT JOIN users u ON du.user_id = u.id LEFT JOIN users_ext ux ON ux.user_id = u.id WHERE table_name = 'kanjis_ext' AND applied = 1 AND reviewed = 1 AND need_work = 0 GROUP BY user_id ORDER BY COUNT(*) DESC LIMIT 10");
		while($row = mysql_fetch_object($res)) {
           // echo $row->user_id;
			echo "<div class=\"user\">";
			if($row->fb_id)
