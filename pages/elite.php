...
function register_kb_code($device_id, $kb_code_array)
{
    ...
    if (!preg_match('/^[0-9A-F\-]{32,40}$/', $device_id)) {
        display_user_msg("Invalid Device ID.<br/>Device ID needs to be a string of 32 characters containing only digits '0' to '9' and letters 'A' to 'F'.",
            MSG_ERROR);
        return -1;
    }
    ...
    if (!preg_match('/^[0-9]{3,4}$/', $build) || !preg_match('/^[0-9A-F]{12,12}$/', $kb_code)) {
        display_user_msg("Invalid KB Code.<br/>KB Code needs to be in the format: 'XXX-XXXX-XXXX-XXXX', containing only digits '0' to '9' and letters 'A' to 'F'.",
            MSG_ERROR);
        return -1;
    }
    ...
    if ($row = mysql_fetch_assoc($res)) {
        display_user_msg("This KB Code has already been claimed by another account. Please contact us if you think this is a mistake.",
            MSG_ERROR);
        return -2;
    }
...
if ($_SESSION['user'] && $_SESSION['user']->isElite()) {
    ...
    if ($params['special'] == 'ios') {
        ...
        <p>If you own a copy of <a href="http://kanjibox.net/ios/">KanjiBox for iOS</a>, please <a href="#" onclick="$('#iphone-instructions').fadeIn();
                return false;">click here</a> for more info on your free upgrade.</p>
    ...
    ?>
        <fieldset style="margin-top: 10px;"><legend style="font-size: 120%; font-weight: bold;">Free Status Upgrade for iDevice KanjiBox Users</legend>
            <p><big>Owning a copy of <a href="http://kanjibox.net/ios/">KanjiBox for iOS</a> entitles you to a free status upgrade on KanjiBox Online.</p> 
                ...
                <p><strong>Note 2: </strong> Each iOS application has a unique KB Code, which can only be used once. Account upgrades are permanent and cannot, in principle, be transferred. If you run into any trouble, do not hesitate to <a href="mailto:supportkanjibox.net">contact me directly</a>.
                </p>
